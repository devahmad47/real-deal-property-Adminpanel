...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    updateUserStatus: (state, action) => {
      ...
      const userIndex = newState.users.findIndex((user) => user._id === userid);

      console.log("a gia")
      if (userIndex !== -1) {
        console.log("ho gia")
        newState.users[userIndex].status = status
      }
      return newState;
    },
    ...
    removeAd: (state, action) => {
      ...
      const newState = JSON.parse(JSON.stringify(state));
      console.log(newState.Ads, _id);
      const PostIndex = newState.Ads.findIndex((user) => user._id === _id);

      if (PostIndex !== -1) {

        newState.Ads = newState.Ads.filter(obj => obj._id !== _id)
      }
      return newState;
    },
...
export const selectAllActiveUsers = (state) => state.auth.activeUsers;


export const { updateUserStatus, allActiveUsers, addAd, removeAd, allAdsStore, login, logout, allusers, removeUser, allPosts, allBumperPosts, removePosts, updatePostStatus, addPinnedPosts, removePinnedPosts, updateBumperPostStatus, addNewPosts, updatePost, BumpPost } = authSlice.actions;
