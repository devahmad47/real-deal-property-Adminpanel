...
import { removeUser, removePosts, removePinnedPosts,removeAd } from "../../Store/authSlice";
            ...
            <ModalFooter>
                <Button color="danger" onClick={async () => {
                    ...
                    try {
                        ...
                        }else if (props.deleteWhat === 'ads') {
                            setloading(true)
                            console.log("delete Ad");
                            let response = await axios.delete(`${serverURL}/api/tasks/delete-tasks/${props.deltedId}`)
                            if (response && response.status === 200) {
                                setloading(false)
                                console.log(response);
                                toast.success(response.data.message)
                                dispatch(removeAd({ _id: props.deltedId }))
                                props.setDeletedId('')
